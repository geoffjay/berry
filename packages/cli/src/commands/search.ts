...
export default class Search extends BaseCommand {
  ...
  async run(): Promise<void> {
    ...
    const tags = flags.tags
      ? flags.tags.split(',').map((tag: string) => tag.trim()).filter(Boolean)
      : undefined;
   ...
   */
  private formatResultChoice(result: SearchResult): { value: SearchResult; name: string; description: string } {
